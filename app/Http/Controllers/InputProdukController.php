...
class InputProdukController extends Controller
{
    ...
    public function simpan_input_produk(Request $request)
    {
        try {
            ...
            //Commit Transaction
            DB::commit();
            return redirect()->back()->with('message', 'Data produk berhasil di

input');
        } catch (Exception $e) {
            //rollback Transaction
            DB::rollback();
            return redirect()->back()->with('error', 'Data gagal di input, silahkan
...
coba lagi!');
    ...
    public function api_id_produk(Request $request)
    {
        try {
            $id_produk = $request->id_produk;
            $data_produk = DB::table('produk')
